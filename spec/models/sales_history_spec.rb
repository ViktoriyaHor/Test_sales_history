...
RSpec.describe SalesHistory, type: :model do
  ...
  describe 'method self.by_month(year)' do
    4.times do |i|
      i += 1
      let!(:"sales_history_#{i}") { create(:sales_history, trans_total_extax_value: 10, trans_date: Date.new(2019,12,3) + i ) }
    end
    it "data is returned with a valid parameter" do
      expect(SalesHistory.by_month(2019)).to eq({12=>40})
    end
    it "data is returned with a parameter nill" do
